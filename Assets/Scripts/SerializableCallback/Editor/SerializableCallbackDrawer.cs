...
public class SerializableCallbackDrawer : PropertyDrawer {
	...
	protected void MethodSelector (SerializedProperty property) {
	...
	protected string PrettifyMethod (string methodName, Type[] parmTypes) {
		string parmnames = PrettifyTypes(parmTypes);
		return methodName + "(" + parmnames + ")";
	}

	protected string PrettifyMethod (MethodInfo methodInfo) {
	...
	protected MethodInfo GetMethod(object target, string methodName, Type[] types) {
	...
	protected Type[] GetArgTypes(SerializedProperty argProp) {
